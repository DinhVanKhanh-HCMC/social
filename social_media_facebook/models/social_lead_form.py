    ...
    def _sync_facebook_leads(self):
        ...
        if isinstance(response, dict) and response.get("data"):
            ...
            if failed_count == 0:
                ...
                _logger.warning(f"{failed_count} leads failed to process, last_sync_at not updated")
            ...
            if failed_count > 0:
                return {
                    ...
                    "params": {
                        ...
                        "message": f"Synced {created_count} leads, {failed_count} failed. Check logs for details.",
                        "type": "warning",
                        "sticky": True,
                    },
                }
...
class SocialLead(models.Model):
    ...
    _sql_constraints = [
        ("fb_lead_id_unique", "unique(fb_lead_id)", "This Facebook lead is already synced!")
    ]

    def action_create_crm_lead(self):
        """Override: Add Facebook-specific logic for CRM lead creation"""
        # Filter Facebook leads
        facebook_leads = self.filtered(lambda l: l.platform == "facebook")
    ...
    def _create_facebook_crm_lead(self):
        ...
        try:
            ...
            _logger.debug(f"Created CRM lead {crm_lead.id} from Facebook lead {self.fb_lead_id}")
