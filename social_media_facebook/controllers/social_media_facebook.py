...
class SocialMediaFacebookController(http.Controller):
    ...
    def facebook_callback(self, **kwargs):
        ...
        if authorization_code:
            ...
            _logger.debug(f"Token response: {token if not isinstance(token, dict) else 'dict with access_token'}")

            if isinstance(token, dict):
                user_access_token = token.get("access_token")
                _logger.debug(f"User access token received: {user_access_token[:20] if user_access_token else 'None'}...")
                ...
                if pages:
                    ...
                    for page in pages:
                        ...
                        _logger.debug(f"Created wizard line ID: {line.id}, selected: {not already_connected}, already_connected: {already_connected}")
