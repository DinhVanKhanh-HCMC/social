...
class WizardFacebookSystemUser(models.TransientModel):
    ...
    def action_connect_system_user(self):
        ...
        if not token:
            raise ValidationError("System User Token is required")
        ...
        try:
            ...
            _logger.error(
                f"Error validating system user token: {e}"
            )
            raise ValidationError(
                "Invalid System User Token or network error."
            )
        ...
        try:
            ...
            raise ValidationError(
                "Failed to fetch pages using the provided "
                "System User Token."
            )
        
        if not pages:
            raise ValidationError(
                "No Facebook pages found for the provided "
                "System User Token."
            )
        ...
        for page in pages:
            ...
            try:
                with self.env.cr.savepoint():
                    ...
                    try:
                        pic = self.env["social.account"]._download_facebook_page_picture(
                            page_id, page_token
                        )
                        if pic:
                            vals["image_1920"] = pic
        ...
        if failed:
            msg = self.env._(f"Created : {len(created)}, Updated : {len(updated)}, Failed : {len(failed)} accounts.")
